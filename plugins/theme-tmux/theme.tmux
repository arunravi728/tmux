...
main() {
  local theme
  theme="$(get_tmux_option "@theme_flavour" "mocha")"
  ...
  # variables.
  # shellcheck source=theme-frappe.tmuxtheme
  source /dev/stdin <<<"$(sed -e "/^[^#].*=/s/^/local /" "${PLUGIN_DIR}/theme-${theme}.tmuxtheme")"
  ...
  # NOTE: Checking for the value of @theme_window_tabs_enabled
  local wt_enabled
  wt_enabled="$(get_tmux_option "@theme_window_tabs_enabled" "off")"
  readonly wt_enabled
  ...
  local user
  user="$(get_tmux_option "@theme_user" "off")"
  readonly user

  local host
  host="$(get_tmux_option "@theme_host" "off")"
  readonly host

  local date_time
  date_time="$(get_tmux_option "@theme_date_time" "off")"
  readonly date_time
 ...
 #setw -g window-status-current-format "#[fg=colour232,bg=$thm_orange] #I #[fg=colour255,bg=colour237] #(echo '#{pane_current_path}' | rev | cut -d'/' -f-2 | rev) "
  ...
  # NOTE: With the @theme_window_tabs_enabled set to on, we're going to
